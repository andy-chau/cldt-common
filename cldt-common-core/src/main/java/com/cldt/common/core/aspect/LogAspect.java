...
public class LogAspect {
	 ...
	 * Log annotation.
	 */
	@Pointcut("@annotation(com.cldt.common.core.annotation.LogAnnotation)")
	...
	private void handleLog(final JoinPoint joinPoint, final Object result) {
		...
		try {
			...
			threadLocal.remove();
			taskExecutor.execute(() -> this.restTemplate.postForObject("http://cldt-provider-uac-service/auth/saveLog", operationLogDto, Integer.class));
		} catch (Exception ex) {
